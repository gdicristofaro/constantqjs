  ...
  private onWorkerInit(
    ...
    worker.onmessage = (evt: MessageEvent) => {
      const {
        ...
        frameStart,
        frameCount,
      } = this.parseMessage(evt.data, outputData, graphMax);
      graphMax = newGraphMax;

      // Precompute per-note keyboard intensities for this batch of frames
      // this has a slight issue as graphMax may be updated within the batch, but it allows us to avoid a separate post-processing step after analysis completes
      ...
      for (let f = frameStart; f < frameStart + frameCount; f++) {
        ...
        for (let i = 0; i < numNotes; i++) {
          relMax = Math.max(relMax, frame[2 * i] ?? 0);
        }
